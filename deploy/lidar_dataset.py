...
class cloudData():
    ...
    def check_dataset(self, localDir, file_infos):
        for i in range(len(file_infos)):
            file_cos_key = file_infos[i]["Key"]
            cos_key = re.search(rf'{os.path.basename(self.prefix)}(.*)', file_infos[i]["Key"]).group(0)
            local_path= localDir + '/' + cos_key
            if not os.path.exists(local_path):
                print(f'download local path: {local_path}')
                self.download_file_from_bucket(file_cos_key, local_path)
...
def get_parma():
    ...
    parser.add_argument('--upload_single_file', action="store_true", default=False, help="will upload single local file if present (default: local folder)")
    parser.add_argument('--download_single_file', action="store_true", default=False, help="will download single file if present (default: local folder)")
...
if __name__ == "__main__":
    ...
    if args.upload_single_file:
        cld.up_file_to_bucket(os.path.join(args.prefix, os.path.basename(args.dir_path)), args.dir_path)
    elif args.download_single_file:
        cld.download_file_from_bucket(args.prefix, args.dir_path)
    else:
        if args.upload:
            ...
            cld.check_dataset(args.dir_path, l)
            print(len(l))
